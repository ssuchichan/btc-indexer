...
///
/// * truncate the block if something is already at `height` (reorg)
/// * refuse (panic) to insert something higher than next empty `height`
...
/// data is just a fixed offset from its id
